...
const LEGACY_SESSIONS = [
  { time: "09:00", title: "Rritja e efikasitetit profesional përmes edukimit, integrimit dhe inovacionit", description: "Sesioni 1" },
  { time: "11:00", title: "Farmacia që përshtatë kujdesin shëndetësor përmes qasjes individuale ndaj pacientit", description: "Sesioni 2" },
  { time: "13:00", title: "Zhvillimi i qëndrueshëm i profesionit të farmacistit- marrja e përgjegjësive të reja profesionale", description: "Sesioni 3" },
  { time: "15:00", title: "Qëndrueshmëria ligjore dhe institucionale e farmacistit", description: "Sesioni 4" },
];
...
export default async function HomePage() {
  let conference = await getConferenceBySlug(CONFERENCE_SLUG);
  const settings = await getSiteSettings();

  // Fallback if conference is missing in DB
  if (!conference) {
    conference = {
      id: "legacy",
      ...
      venue_city: "Prishtinë",
      venue_address: "Prishtinë, Kosovë",
  ...
  if (typeof agenda === "string") {
    try { agenda = JSON.parse(agenda); } catch (e) { agenda = []; }
  }
  if (!Array.isArray(agenda) || agenda.length === 0) {
    agenda = LEGACY_SESSIONS;
  }
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-10 text-sm font-medium text-slate-700">
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border">
              ...
              {conference.start_date ? new Date(conference.start_date).toLocaleDateString('sq-AL', { day: 'numeric', month: 'long', year: 'numeric' }) : "13 Dhjetor 2025"}
            </div>
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border">
              <Clock className="h-4 w-4 text-blue-600" />
              {conference.start_date ? new Date(conference.start_date).toLocaleTimeString('sq-AL', { hour: '2-digit', minute: '2-digit' }) : "08:30"}
            </div>
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border">
              <MapPin className="h-4 w-4 text-blue-600" />
              {conference.location || "Hotel Emerald"}, {conference.venue_city || "Prishtinë"}
            </div>
          </div>
          ...
          <div className="grid items-start gap-10 lg:grid-cols-2">
            <div className="space-y-6">
              ...
              <p className="text-lg leading-relaxed text-slate-700">
                Konferenca e sivjetme, në kuadër të 60-vjetorit të Shoqatës Farmaceutike të Kosovës, do të shërbejë si një platformë
                për të diskutuar të ardhmen e profesionit tonë. Do të eksplorojmë se si edukimi i vazhdueshëm, inovacioni teknologjik
                dhe qëndrueshmëria institucionale mund të formësojnë një të ardhme më të ndritur për farmacistët dhe pacientët në Kosovë.
              </p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <Card className="border-green-100 bg-green-50/60">
                  <CardContent className="p-4">
                    ...
                    <p className="mt-1 font-semibold text-slate-900">{conference.location || "Hotel Emerald"}</p>
                    <p className="text-sm text-slate-600">{conference.venue_city || "Prishtinë"}</p>
                  </CardContent>
                </Card>
            ...
            <div className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-6 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-slate-500">4 tema</span>
              </div>
              <div className="space-y-3">
                {LEGACY_SESSIONS.map((session, index) => (
                  <div key={session.title} className="flex items-start gap-3 rounded-lg bg-white/60 p-3 shadow-sm">
          ...
          <div className="grid gap-10 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 space-y-3 text-left text-sm text-slate-700">
                ...
                <p>• Vendi i konferencës: {conference.location || "Hotel Emerald"}, {conference.venue_city || "Prishtinë"}</p>
